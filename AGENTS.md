...
- 取整话拆页后，以 2% 比例容差聚类并选择页数最多的常见比例簇，以簇内最窄比例作为正常页面目标；只向上下补黑边，不能左右补边或裁切。比常见比例更高的异常长页保持自身高度，不能反过来拉长整话其他页面。变化影响目标比例时，相关页面需要重新生成。
- Sharp 单次 `extend` 的每侧上限为 10000 像素；补边统一使用黑色画布与 raw 页面合成，不能因超长图裁切或改变全话目标比例，也不能引入中间格式编码。
