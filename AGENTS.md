...
- 标签筛选在左，作者在右。作者可搜索单选；标签可搜索多选且只能选已有标签，多个标签按“与”过滤。筛选保存在浏览器当前页面的内存中，站内往返时保留，刷新页面后重置。
...
- 编辑标题时，未覆盖的值作为 placeholder，输入框为空；只显示扫描值，不使用“未填写（默认：xxx）”。发布时间使用日期选择器，导入时间使用日期时间选择器。编辑表单存在未保存修改时，关闭主弹层必须二次确认是否放弃。
...
- 标题使用 `scanned_title` 和 `title_override` 区分扫描值与手动值；二层扫描只为空作者补入第一层目录名，不覆盖非空作者。手动标题、作者、日期、标签、封面及章节标题不能被刷新覆盖；`manual_tags` 标识手动标签，语义切换前产生的扫描标签仅在该值为 0 时清除。
...
- 先按 EXIF 校正方向，宽大于高视为双页，先右半页后左半页；宽大于高两倍视为四页，等分后从最右到最左输出。宽度不能被分片数整除时也要完整保留像素。
...
- 服务日志使用单行 JSON，包含 UTC 时间、级别、稳定事件名、中文消息和必要上下文，方便 Docker 查看及日志采集。
