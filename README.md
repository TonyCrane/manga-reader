...
一个面向 NAS 的自托管漫画阅读器。从只读目录导入漫画，在浏览器中提供书架、元信息管理、用户权限，以及适合手机和平板的分页与滚动阅读。

![书架：搜索、筛选和漫画信息](docs/screenshots/library.png)

![横屏阅读：跨页还原、标题和阅读进度](docs/screenshots/reader-wide.png)
